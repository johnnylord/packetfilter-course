...
int xdp_icmp_dos_mitigation_func(struct xdp_md *ctx)
{
	...
	bpf_printk("source mac: %llu, dest mac: %llu\n", ether_addr_to_u64(eth->h_source), ether_addr_to_u64(eth->h_dest));
	bpf_printk("source ip: %lu, dest ip: %lu\n", bpf_ntohs(iphdr->saddr), bpf_ntohs(iphdr->daddr));
